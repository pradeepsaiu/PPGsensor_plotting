...
ISR(TIMER2_COMPA_vect){                         // triggered when Timer2 counts to 124
  ...
  Signal2 = analogRead(pulsePin2);            // read the Pulse Sensor 2
  ...
  // signal surges up in value every time there is a pulse
  if (N > 250){                                   // avoid high frequency noise
    if ( (Signal2 > thresh2) && (Pulse == false) && (N > (IBI/5)*3) ){        
      Pulse = true;                               // set the Pulse flag when we think there is a pulse
      digitalWrite(blinkPin,HIGH);                // turn on pin 13 LED
      IBI = sampleCounter - lastBeatTime;         // measure time between beats in mS
      lastBeatTime = sampleCounter;               // keep track of time for next pulse
  ...
  if (Signal2 < thresh2 && Pulse == true){   // when the values are going down, the beat is over
    digitalWrite(blinkPin,LOW);            // turn off pin 13 LED
    Pulse = false;                         // reset the Pulse flag so we can do it again
    amp = P2 - T2;                           // get amplitude of the pulse wave
    thresh2 = amp/2 + T;                    // set thresh at 50% of the amplitude
